  ...
  for (var i in props) {
    ...
    if (i[0] == "$") {
      ...
      if (i == "$if") {
        ...
        i == "$value" &&
        ["input", "textarea", "select"].indexOf(this.tag) > -1
      )
        this.bindDom(o, "value");
      else if (i == "$checked" && this.tag == "input")
        this.bindDom(o, "checked");
...
Node.prototype.bindDom = function (value, domUpdater) {
  ...
  const updateDom =
    typeof domUpdater == "string"
      ? (el, v) => (el[domUpdater] = v)
      : domUpdater;
